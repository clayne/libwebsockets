...
normal operation, but also to be able to easily confirm that they act correctly
under various fault conditions that may be difficult to arrange at test-time.

Code handling the failures we want to check may be anywhere, including during
early initialization or in user code before lws intialization.
...
The basic idea is the user code can choose to instantiate "fault contexts" that
list named, well-known "faults" that the code supoorts and that the user wants
to inject.  These "fault contexts" can be directly initialized at object
creation time, eg, the lws_context creation, or a client connection or Secure
Stream, and may use a namespace path to target downstream objects created
later, and force fake faults where the code checks if a named fault should
happen.
...
and one or more of these, eg on the stack
...
|**ss=mystream/**subrule|subrule is inherited by SS of streamtype "mystream" when it is created by object with this rule|
|**wsi/**subrule|subrule is inherited by wsi created by object with this rule|

Namespaces can be combined, for example `vh=myvhost/wsi/sockfail` will set the
`sockfail` fault on wsi created by the vhost "myvhost", ie, it will cause the
listen socket for the vhost to error out on creation.
...
|Syntax|Meaning|
|---|---|
|`wsi/dnsfail`|Inject the fault every time|
|`wsi/dnsfail(10%)`|Randomly inject the fault at 10% probability|
|`wsi/dnsfail(.............X.X)`|Inject the fault on the 14th and 16th try, every 16 tries|
...
|context|`ctx_createfail_evlib_plugin`|Fail context creation due to event lib plugin failed init|
...
|context|`ctx_createfail_evlib_init`|Fail context creation due to event lib init failed (if evlib enabled)|
|context|`ctx_createfail_evlib_pt`|Fail context creation due to event lib pt init failed (if evlib enabled)|
...
|srv ss|`ss_srv_vh_fail`|Secure Streams Server vhost creation forced to fail|
|cli ss|`ss_no_streamtype_policy`|The policy for the streamtype is made to seem as if it is missing|
|srv vhost|`listenskt`|Causes `socket()` allocation for vhost listen socket to fail|
|cli wsi|`dnsfail`|Sync: `getaddrinfo()` is not called and a EAI_FAIL return synthesized, Async: request not started and immediate fail synthesized|
|cli wsi|`sendfail`|Attempts to send data on the wsi socket fail|
|cli wsi|`connfail`|Attempts to connect on the wsi socket fail|
|cli wsi|`createfail`|Creating the client wsi itself fails|
|udp wsi|`udp_rx_loss`|Drop UDP RX that was actually received, useful with probabalistic mode|
|udp wsi|`udp_tx_loss`|Drop UDP TX so that it's not actually sent, useful with probabalistic mode|
...
At client connection creation time, user code can also specify
